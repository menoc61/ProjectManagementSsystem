...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $commentairesAffectation = $_POST['commentairesAffectation'];
    
    // Vérifier si cette affectation existe déjà
    $checkSql = "SELECT * FROM AFFECTATION WHERE IdPersonnel = $idPersonnel AND IdTache = $idTache";
    $checkResult = mysqli_query($conn, $checkSql);
    
    if (mysqli_num_rows($checkResult) > 0) {
        $error = "Cette affectation existe déjà.";
    } else {
        $sql = "INSERT INTO AFFECTATION (IdPersonnel, IdTache, DateAffectation, CommentairesAffectation) 
                VALUES ('$idPersonnel', '$idTache', '$dateAffectation', '$commentairesAffectation')";
                ...
                <div class="card">
                    <div class="card-body">
                        <form method="post" action="create.php">
                            <div class="mb-3">
                                <label for="idPersonnel" class="form-label">Personnel *</label>
                                <select class="form-select" id="idPersonnel" name="idPersonnel" required>
                                    ...
                                    ?>
                                    <option value="<?php echo $personnel['IdPersonnel']; ?>" <?php echo $preselectedPersonnel == $personnel['IdPersonnel'] ? 'selected' : ''; ?>>
                            ...
                            <div class="mb-3">
                                <label for="commentairesAffectation" class="form-label">Commentaires</label>
                                <textarea class="form-control" id="commentairesAffectation" name="commentairesAffectation" rows="3"></textarea>
                            </div>
