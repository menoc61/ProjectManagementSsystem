...
$where = [];
if (!empty($search)) {
    $where[] = "(NomUtilisateur LIKE '%$search%' OR EmailUtilisateur LIKE '%$search%')";
}
if (!empty($role)) {
    $where[] = "RoleUtilisateur = '$role'";
}
                        ...
                        <tbody>
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    echo "<td>" . htmlspecialchars($row['EmailUtilisateur']) . "</td>";
                                    echo "<td>";
                                    if ($row['RoleUtilisateur'] == 'admin') {
                                        echo "<span class='badge bg-danger'>Administrateur</span>";
                                    } else {
                                        echo "<span class='badge bg-primary'>Personnel</span>";
                                    }
                                    echo "</td>";
                                    echo "<td>" . ($row['DerniereConnexion'] ? date('d/m/Y H:i', strtotime($row['DerniereConnexion'])) : 'Jamais') . "</td>";
                                    echo "<td>";
