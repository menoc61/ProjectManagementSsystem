...
class PDF {
    ...
    public function addSummary($totalProjets, $totalReglements, $solde) {
        ...
        <div class="summary">
            ...
            <p><strong>Total des projets:</strong> ' . number_format($totalProjets, 2, ',', ' ') . ' €</p>
            <p><strong>Total des règlements:</strong> ' . number_format($totalReglements, 2, ',', ' ') . ' €</p>
...
if ($clientId) {
    ...
    if (mysqli_num_rows($projetsResult) > 0) {
        $pdf->startTable(['ID', 'Titre', 'Type', 'Coût (€)', 'Début', 'Fin', 'État']);
    ...
    if (mysqli_num_rows($reglementsResult) > 0) {
        $pdf->startTable(['ID', 'Date', 'Heure', 'Montant (€)']);
...
// Sinon, générer un PDF avec tous les clients
    ...
    if (mysqli_num_rows($result) > 0) {
        $pdf->startTable(['ID', 'Nom', 'Email', 'Téléphone', 'Projets', 'Total Projets (€)', 'Total Règlements (€)', 'Solde (€)']);
